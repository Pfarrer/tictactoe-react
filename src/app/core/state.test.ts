...
import { reducer, type Board, type GameMode, type GameState } from './state';
...
describe('state', () => {
  it('game finsihed when there are no empty cells left', () => {
    ...
    const updatedState = reducer(initialState, { type: "computer_move_requested", cellIdx: 8 });
    expect(updatedState.gameStatus).toBe("finished");
    expect(updatedState.winner).toBeUndefined();
  });
});

function initActiveGameState(board: Board, nextTurn: GameState["nextTurn"], gameMode: GameMode = "Human-vs-Computer"): GameState {
  return {
    board,
    gameMode,
    gameStatus: "active",
